...
const EMPTY_OBJ = !!(process !== "production") ? Object.freeze({}) : {};
const EMPTY_ARR = !!(process !== "production") ? Object.freeze([]) : [];
const NOOP = () => {};
const NO = () => false;
const onRE = /^on[^a-z]/;
...
let activeEffect;
const ITERATE_KEY = Symbol(!!(process !== "production") ? "iterate" : "");
const MAP_KEY_ITERATE_KEY = Symbol(
  !!(process !== "production") ? "Map key iterate" : ""
);
...
function track(target, type, key) {
  if (shouldTrack && activeEffect) {
    ...
    const eventInfo = !!(process !== "production")
...
function trackEffects(dep, debuggerEventExtraInfo) {
  ...
  if (shouldTrack2) {
    ...
    if (!!(process !== "production") && activeEffect.onTrack) {
...
function triggerEffect(effect2, debuggerEventExtraInfo) {
  if (effect2 !== activeEffect || effect2.allowRecurse) {
    if (!!(process !== "production") && effect2.onTrigger) {
...
function clear() {
  ...
  const oldTarget = !!(process !== "production")
    ? isMap(target)
      ? new Map(target)
      : new Set(target)
    : void 0;
...
function computed$1(getterOrOptions, debugOptions, isSSR = false) {
  ...
  if (!!(process !== "production") && debugOptions && !isSSR) {
...
function handleError(err, instance, type, throwInDev = true) {
  ...
  if (instance) {
    ...
    const errorInfo = !!(process !== "production")
...
function flushPreFlushCbs(seen, i = isFlushing ? flushIndex + 1 : 0) {
  ...
  for (; i < queue.length; i++) {
    const cb = queue[i];
    if (cb && cb.pre) {
      if (!!(process !== "production") && checkRecursiveUpdates(seen, cb)) {
        continue;
      }
      queue.splice(i, 1);
      i--;
...
function flushJobs(seen) {
  ...
  const check = !!(process !== "production")
  ...
  try {
    for (flushIndex = 0; flushIndex < queue.length; flushIndex++) {
      ...
      if (job && job.active !== false) {
        if (!!(process !== "production") && check(job)) {
          continue;
        }
...
function renderComponentRoot(instance) {
  ...
  try {
    if (vnode.shapeFlag & 4) {
      ...
      if (!!(process !== "production") && attrs === props) {
        markAttrsAccessed();
      }
      result = normalizeVNode(
        render2.length > 1
  ...
  if (vnode.dirs) {
    if (!!(process !== "production") && !isElementRoot(root)) {
      warn(
...
function watch(source, cb, options) {
  if (!!(process !== "production") && !isFunction(cb)) {
    warn(
...
function doWatch(
  ...
  if (isRef(source)) {
    ...
    getter = () =>
      source.map((s) => {
        if (isRef(s)) {
          ...
          !!(process !== "production") && warnInvalidSource(s);
        }
      });
    ...
    getter = NOOP;
    !!(process !== "production") && warnInvalidSource(source);
  }
  if (cb && deep) {
...
const publicPropertiesMap =
  ...
  /* @__PURE__ */ extend(/* @__PURE__ */ Object.create(null), {
    ...
    $data: (i) => i.data,
    $props: (i) =>
      !!(process !== "production") ? shallowReadonly(i.props) : i.props,
    $attrs: (i) =>
      !!(process !== "production") ? shallowReadonly(i.attrs) : i.attrs,
    $slots: (i) =>
      !!(process !== "production") ? shallowReadonly(i.slots) : i.slots,
    $refs: (i) =>
      !!(process !== "production") ? shallowReadonly(i.refs) : i.refs,
...
const PublicInstanceProxyHandlers = {
  get({ _: instance }, key) {
    const { ctx, setupState, data, props, accessCache, type, appContext } =
      instance;
    if (!!(process !== "production") && key === "__isVue") {
      return true;
    }
    let normalizedProps;
    if (key[0] !== "$") {
    ...
    if (publicGetter) {
      if (key === "$attrs") {
        ...
        !!(process !== "production") && markAttrsAccessed();
      } else if (!!(process !== "production") && key === "$slots") {
        track(instance, "get", key);
      }
...
function applyOptions(instance) {
  ...
  const checkDuplicateProperties = !!(process !== "production")
  ...
  if (dataOptions) {
    if (!!(process !== "production") && !isFunction(dataOptions)) {
      warn(
        `The data option must be a function. Plain object usage is no longer supported.`
      );
    }
    const data = dataOptions.call(publicThis, publicThis);
    if (!!(process !== "production") && isPromise(data)) {
      warn(
        `data() returned a Promise - note data() cannot be async; If you intend to perform data fetching before component renders, use async setup() + <Suspense>.`
      );
    }
    if (!isObject(data)) {
      !!(process !== "production") && warn(`data() should return an object.`);
    } else {
  ...
  if (computedOptions) {
    for (const key in computedOptions) {
      ...
      const get = isFunction(opt)
        ...
        : NOOP;
      if (!!(process !== "production") && get === NOOP) {
  ...
  return function createApp(rootComponent, rootProps = null) {
    ...
    const app = (context.app = {
      ...
      component(name, component) {
        ...
        if (!!(process !== "production") && context.components[name]) {
          warn(
      ...
      directive(name, directive) {
        ...
        if (!!(process !== "production") && context.directives[name]) {
          warn(
      ...
      mount(rootContainer, isHydrate, isSVG) {
        if (!isMounted) {
          if (!!(process !== "production") && rootContainer.__vue_app__) {
            warn(
...
If you want to remount the same app, move your app creation logic into a factory function and create fresh app instances for each mount - e.g. \`const createMyApp = () => createApp(App)\``
      ...
      provide(key, value) {
        if (!!(process !== "production") && key in context.provides) {
          warn(
...
function updateProps(instance, rawProps, rawPrevProps, optimized) {
  ...
  if (
    ...
    !(!!(process !== "production") && isInHmrContext(instance)) &&
...
function normalizePropsOptions(comp, appContext, asMixin = false) {
  ...
  if (isArray(raw)) {
    for (let i = 0; i < raw.length; i++) {
      if (!!(process !== "production") && !isString(raw[i])) {
    ...
    if (!!(process !== "production") && !isObject(raw)) {
...
const normalizeSlot = (key, rawSlot, ctx) => {
  ...
  const normalized = withCtx((...args) => {
    if (!!(process !== "production") && currentInstance) {
      warn(
...
const normalizeVNodeSlots = (instance, children) => {
  if (!!(process !== "production") && !isKeepAlive(instance.vnode) && true) {
    warn(
...
const updateSlots = (instance, children, optimized) => {
  ...
  if (vnode.shapeFlag & 32) {
    const type = children._;
    if (type) {
      if (!!(process !== "production") && isHmrUpdating) {
...
function initFeatureFlags() {
  const needWarn = [];
  if (!!(process !== "production") && needWarn.length) {
...
function baseCreateRenderer(options, createHydrationFns) {
  ...
  const patch = (
    ...
    isSVG = false,
    slotScopeIds = null,
    optimized = !!(process !== "production") && isHmrUpdating
      ? false
      : !!n2.dynamicChildren
  ) => {
    if (n1 === n2) {
  ...
  const processFragment = (
    ...
    if (
      !!(process !== "production") && // #5523 dev root fragment may inherit directives
  ...
  const mountComponent = (
    ...
    if (!!(process !== "production") && instance.type.__hmrId) {
...
function traverseStaticChildren(n1, n2, shallow = false) {
  ...
  if (isArray(ch1) && isArray(ch2)) {
    for (let i = 0; i < ch1.length; i++) {
      ...
      if (!!(process !== "production") && c2.type === Comment && !c2.el) {
        c2.el = c1.el;
      }
    }
  }
...
function createBaseVNode(
  ...
  if (!!(process !== "production") && vnode.key !== vnode.key) {
...
const createVNode = !!(process !== "production")
...
function _createVNode(
  ...
  const shapeFlag = isString(type)
    ...
    ? 2
    : 0;
  if (!!(process !== "production") && shapeFlag & 4 && isProxy(type)) {
    type = toRaw(type);
    warn(
...
function cloneVNode(vnode, extraProps, mergeRef = false) {
  ...
  const cloned = {
    ...
    children:
      !!(process !== "production") && patchFlag === -1 && isArray(children)
  ...
  if (setup) {
    ...
    if (isPromise(setupResult)) {
      ...
      if (isSSR) {
        ...
        if (!!(process !== "production") && !instance.suspense) {
...
function handleSetupResult(instance, setupResult, isSSR) {
  if (isFunction(setupResult)) {
    ...
    if (!!(process !== "production") && isVNode(setupResult)) {
      warn(
  ...
  } else if (!!(process !== "production") && setupResult !== void 0) {
    warn(
...
function initCustomFormatter() {
  if (!!!(process !== "production") || typeof window === "undefined") {
    return;
  }
...
function patchDOMProp(
  ...
  try {
    ...
    if (!!(process !== "production") && !needRemove) {
      warn(
...
class VueElement extends BaseClass {
  constructor(_def, _props = {}, hydrate2) {
    ...
    if (this.shadowRoot && hydrate2) {
      ...
      if (!!(process !== "production") && this.shadowRoot) {
        warn(
...
const VueHelloWorld = /*#__PURE__*/ _export_sfc(_sfc_main, [
  ["render", _sfc_render],
]);

const HelloWorld = defineCustomElement(VueHelloWorld);

function register(tagName = "hello-world") {
  customElements.define(tagName, HelloWorld);
}

export { HelloWorld, register };
